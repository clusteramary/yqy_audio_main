...
class IntegratedReceiver:
    ...
    def __init__(self, emotion_host="127.0.0.1", emotion_port=5555, voice_host="127.0.0.1", voice_port=5557):
        ...
        # ★ 新增：指令“粘滞保持”（防止瞬时回落）
        self._last_cmd_index = None  # 最后一个命令的索引
        self._voice_timeout = 8.0  # 秒
        ...
        while self.running:
            ...
            try:
                ...
                try:
                    ...
                    # 处理“递话筒”和“收话筒”消息
                    elif voice_data.get('type') == 'mic_command':
                        command = voice_data.get('command')
                        if command == "send_microphone":
                            # 递话筒操作，映射到索引 6
                            self._last_cmd_index = 6
                            self._last_cmd_ts = time.time()
                            print(f"[UDP] 收到递话筒指令")
                        elif command == "release_microphone":
                            # 收话筒操作，映射到索引 9
                            self._last_cmd_index = 9
                            self._last_cmd_ts = time.time()
                            print(f"[UDP] 收到收话筒指令")
