...
# === 新增：动作索引常量（与接收端约定） ===
GESTURE_INDEX_MIC_ON = 6     # 拿话筒（第1问发出时）
GESTURE_INDEX_WAVE_END = 7   # 结束挥手（第4问问完且用户开始回答时）
...
class DialogSession:
    ...
    def __init__(
        ...
        # ====== 新增：问答计数与 VAD 触发 ======
        self.question_count = 0                   # 已检测到的问题数（累计到4）
        self.first_question_index_sent = False    # 是否已发“拿话筒”索引
        self.awaiting_answer_after_q4 = False     # 是否等待用户回答第4问
        self.q4_answer_index_sent = False         # “结束挥手”是否已发
        self._vad_threshold = 250                 # 简单能量门限（rms），可按现场调
        self._q4_answer_deadline = 0.0            # 等待回答的超时时间戳（可选）
    ...
    async def _promote_task(self):
        ...
        while self.is_running:
            await asyncio.sleep(2000000)  # 每40秒检查一次
    ...
    def _send_gesture_index(self, idx: int, label: str):
            """通过现有 UDP 通道发送动作索引号"""
            try:
                payload = {
                    "type": "gesture_index",
                ...
                print(f"[GestureIndex] sent idx={idx} label={label}")
            except Exception as e:
                print(f"[GestureIndex] 发送失败: {e}")
    ...
    def handle_server_response(self, response: Dict[str, Any]) -> None:
        ...
        if response["message_type"] == "SERVER_ACK" and isinstance(
            ...
            if 'content' in payload_msg:
                ...
                # 检查关键词并发送UDP消息
                # === 新增：统计问题并在阈值处发索引 ===
                # 简单规则：一条 content 里出现几个问号，就认为有几个问题
                q_in_text = content.count('？') + content.count('?')
                if q_in_text > 0:
                    prev = self.question_count
                    self.question_count = min(4, self.question_count + q_in_text)

                    # 第一次检测到问题 → 发送“拿话筒”（只发一次）
                    if (not self.first_question_index_sent) and self.question_count >= 1:
                        self._send_gesture_index(GESTURE_INDEX_MIC_ON, "mic_on")
                        self.first_question_index_sent = True

                    # 第4问已出现 → 等待用户回答（之后由麦克风 VAD 触发 index=7）
                    if prev < 4 and self.question_count >= 4 and (not self.q4_answer_index_sent):
                        self.awaiting_answer_after_q4 = True
                        self._q4_answer_deadline = time.time() + 60.0  # 最多等60秒，可调整
                        print("[GestureIndex] 第4问已发出，等待用户回答以发送 wave_end")
    ...
    async def process_microphone_input(self) -> None:
        ...
        while self.is_recording:
            try:
                ...
                # 半双工：播放TTS时不读不发，但增加保活静音，防止服务器断开
                if self.block_mic_while_playing and self._is_tts_playing():
                    await self._send_silence_if_due()  # 到点发静音做 keep-alive
                    await asyncio.sleep(0.01)
                    continue
                ...
                while total_len - offset >= frame_bytes:
                    ...
                    # === 新增：在等待第4问回答期间，用简单 VAD 触发 index=7 ===
                    if self.awaiting_answer_after_q4 and (not self.q4_answer_index_sent):
                        try:
                            # 计算该 10ms 包的 RMS（16-bit 单声道）
                            rms = audioop.rms(chunk16k, 2)
                            # 到达门限则认为用户开始回答
                            if rms >= self._vad_threshold:
                                self._send_gesture_index(GESTURE_INDEX_WAVE_END, "wave_end")
                                self.q4_answer_index_sent = True
                                self.awaiting_answer_after_q4 = False
                            # 超时自动放弃等待（可选）
                            elif self._q4_answer_deadline and time.time() > self._q4_answer_deadline:
                                self.awaiting_answer_after_q4 = False
                                print("[GestureIndex] 等待回答超时，取消 wave_end 触发")
