...
class DialogSession:
    ...
    负责：
      ...
      - 通过 mic_command 通道控制“话筒递/收”（索引 9 / 6）
    """
    ...
    def __init__(
        ...
        # MIC 指令冷却（底层 send_mic_command 使用）
        self._last_mic_send_time = 0.0
        # MIC 高层状态：idle / to_user(递话筒, index=9) / to_robot(收话筒, index=6)
        self._mic_state: str = "idle"
        ...
        if not self.is_audio_file_input and _HAS_ROS1:
            ...
            # === 订阅麦克风音频（audio_capture 发布到 /audio/audio） ===
    ...
    # ---------- MIC 高层状态机 ----------
    def _set_mic_state(self, new_state: str):
        """
        高层 MIC 状态机：
          - "to_user"  -> 递话筒 (release_microphone -> index 9)
          - "to_robot" -> 收话筒 (send_microphone   -> index 6)

        只在状态变化时发送指令，并且冗余发几次防止偶发 UDP 丢包。
        """
        if new_state == self._mic_state:
            return

        self._mic_state = new_state

        if new_state == "to_user":
            # 递话筒：MicReceiver 中 release_microphone -> index_release(=9)
            cmd = "release_microphone"
        elif new_state == "to_robot":
            # 收话筒：MicReceiver 中 send_microphone -> index_send(=6)
            cmd = "send_microphone"
        else:
            # idle 或其他暂不发指令
            return

        # 冗余发几次，cooldown 设为 0 避免被节流
        for _ in range(3):
            self.send_mic_command(cmd, cooldown_sec=0.0)
        ...
        while self.is_playing:
            try:
                ...
                if audio_data is not None:
                    ...
                    # 机器人说话阶段 → 递话筒给受访者 (release_microphone -> index 9)
                    self._set_mic_state("to_user")
            except queue.Empty:
                time.sleep(0.1)
            except Exception as e:
                print(f"音频播放错误: {e}")
    ...
    async def process_microphone_input(self) -> None:
        ...
        麦克风输入改为从 ROS 话题 /audio/audio 读取：
          ...
          - 同时：根据是否在播放 TTS，切换话筒“递/收”状态（索引 9 / 6）。
        """
        ...
        while self.is_recording:
            try:
                ...
                # 如果当前没有在播放 TTS，可以认为是“用户说话阶段” → 收话筒
                # 注意：只在 TTS 未播放时从“递话筒”切回“收话筒”，避免和播放线程互抢状态
                if not self._is_tts_playing():
                    self._set_mic_state("to_robot")
