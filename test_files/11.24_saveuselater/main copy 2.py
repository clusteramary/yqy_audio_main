...
from str_receiver import UDPReceiver  # ⭐ 新增：引入 UDPReceiver
...
async def main():
    """
    外层自恢复循环：每次 run_once 结束（含无人脸被看门狗杀掉），立即重新开始新一轮。
    如需“彻底退出”，直接 Ctrl+C 终止进程即可。
    """
    # ========== ⭐ 在这里启动 UDPReceiver，监听控制字符串并写入 ctrl.txt ==========
    receiver = UDPReceiver(
        listen_ip="0.0.0.0",
        listen_port=8889,  # 和你之前示例一致
        file_path=str(CTRL_FILE_PATH),  # 用主程序同一个 ctrl.txt 路径
    )
    recv_thread = threading.Thread(target=receiver.start_receiving, daemon=True)
    recv_thread.start()
    print(
        f"[UDPReceiver] 已在后台启动，监听 {receiver.listen_ip}:{receiver.listen_port}，写入 {receiver.file_path}"
    )
