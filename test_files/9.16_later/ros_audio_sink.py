...
class LocalAudioSink:
    """
    订阅 /audio 并交给 AudioPlayer 播放。
    同时挂两种订阅：AudioData（如可用）与 ByteMultiArray，匹配的那个会收到数据。
    """

    def __init__(self, topic="/audio", sample_rate=24000, channels=1, sample_format="s16le", device_index=None):
        ...
        if HAS_AUDIO_DATA:
            ...
            self.sub_aud = None
            rospy.logwarn("audio_common_msgs/AudioData 不可用，只使用 ByteMultiArray 订阅。")
...
def main():
    ...
    sink = LocalAudioSink(topic=topic, sample_rate=rate, channels=ch,
                          sample_format=fmt, device_index=dev)
    try:
        rospy.spin()
    finally:
        sink.close()
