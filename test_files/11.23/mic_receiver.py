...
class MicReceiver:
    ...
    def __init__(self, host="127.0.0.1", port=5558,
                 ...
                 command_hold_secs: float = 1.0, latch: bool = True):
        self.host = host
        self.port = port
    ...
    def get_mic_index(self) -> int:
        """latch=True: 永远返回最后一次状态（6/9），除非还没收到过 -> 0
           latch=False: 保持脉冲式（超时回落 0）"""
        with self._lock:
            ...
            # 下面是旧的“脉冲式”
            now = time.monotonic()
            if (now - self._last_ts) <= self.command_hold_secs:
                return self._last_index
            return 0
