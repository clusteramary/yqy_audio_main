...
DEFAULT_CONFIG = {
    ...
    "ACTION_HAND_DIR": "/home/zxr/Documents/DA_robot/interview/src/direct_control/scripts",
    # Task env names
    "ENV_DIRECT_CONTROL": "myx_realman",
    "ENV_YQY": "yqy1",
    # Scripts / launch file
    "ACTION_HAND_SCRIPT": "action_hand_photo.py",
    "ACTION_PHOTO_SCRIPT": "action_photo_1216.py",
    "PHOTO_LAUNCH_FILE": "launch/photo.launch",
    "YQY_MAIN_SCRIPT": "main_photo.py",
    # Auto retry (kept as-is; only applied to action_photo)
...
YQY_DIR = CFG["YQY_DIR"]
ACTION_HAND_DIR = CFG["ACTION_HAND_DIR"]
...
class App(tk.Tk):
    def __init__(self):
        ...
        hint = (
            "Notes:\n"
            "• Recommended order: action_hand → action_photo → photo_head → main_photo\n"
            "• action_hand: finishes and exits automatically\n"
            "• action_photo: auto-retry on connection failure (max retries configurable)\n"
            "• main_photo: Stop uses TERM → KILL (for stubborn processes)\n"
            "• Ops: connect laptop to phone hotspot (mobile data), avoid campus network\n"
            "• If speech freezes / stuck: close and relaunch task (Ctrl+C may not work)\n"
    ...
    def _define_tasks(self):
        # 1) action_hand (右手放相机，结束后退出)
        cmd1 = bash_cmd(
            CFG["ENV_DIRECT_CONTROL"],
            f"""
cd "{ACTION_HAND_DIR}"
python "{CFG['ACTION_HAND_SCRIPT']}"
""",
        )
        self.tasks["action_hand"] = ManagedTask(
            name="action_hand",
            cwd=ACTION_HAND_DIR,
            cmd=cmd1,
...
rosrun direct_control "{CFG['ACTION_PHOTO_SCRIPT']}"
""",
        )
        self.tasks["action_photo"] = ManagedTask(
            name="action_photo",
            cwd=ROOT_DIR,
            cmd=cmd2,
            enqueue_log=self.enqueue_log,
        ...
        # 3) photo_head (控制头)
        # NOTE: 按你给的命令，这里不做 conda activate
        cmd3 = f"""
set -e
cd "{ROOT_DIR}"
source ./devel/setup.bash
roslaunch {CFG['PHOTO_LAUNCH_FILE']}
"""
        self.tasks["photo_head"] = ManagedTask(
            name="photo_head",
            cwd=ROOT_DIR,
            cmd=cmd3,
            enqueue_log=self.enqueue_log,
        ...
        cmd4 = bash_cmd(
            ...
            f"""
cd "{YQY_DIR}"
python "{CFG['YQY_MAIN_SCRIPT']}"
""",
        )
        self.tasks["main_photo"] = ManagedTask(
            name="main_photo",
            cwd=YQY_DIR,
            cmd=cmd4,
            enqueue_log=self.enqueue_log,
            stop_mode="termkill",  # TERM -> KILL
        )

    def start_all(self):
        # 按你给的顺序：action_hand 结束后再继续后面三个
        self.tasks["action_hand"].start()

        def _chain_start():
            # 等 action_hand 结束
            while self.tasks["action_hand"].is_running():
                time.sleep(0.1)
            # 再启动后续
            for k in ["action_photo", "photo_head", "main_photo"]:
                self.tasks[k].start()
                time.sleep(0.3)

        threading.Thread(target=_chain_start, daemon=True).start()

    def stop_all(self):
        for k in ["main_photo", "photo_head", "action_photo", "action_hand"]:
            self.tasks[k].stop()
