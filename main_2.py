...
ABSENT_SECONDS = 10.0  # ✅ 对话进行时，连续多久没看到人脸就重启
EMOTION_INTERVAL = 5  # 情绪线程检测频率（越小越灵敏，代价是算力更高）
INITIAL_DETECT_TIMEOUT = 1000.0  # 首次做人脸特征引导的超时时间
...
- 如果是“现在视野中有1个人”，检测为人数减少，说一句：“诶，刚刚那个朋友去哪了？”
- 其余情况检测为人数增多，说一句：“很高兴认识新朋友”
...
# 让Q2/Q3更不“念清单”的融合提示（拼接进每个STYLE，让风格自己消化Q2/Q3）
Q23_BLEND = r"""
【Q2/Q3 融入深挖提示（本风格可用｜为了更自然不念清单）】
- 当对方回答了Q1“愿望”后：
  你可以立刻用追问把Q2/Q3融进去，而不是单独念题。
  例如：
  - （Q2）“这个愿望落到生活里，你最期待先变好的是什么？”
  - （Q3）“如果从第一步开始，你打算先做哪件小事？”
- 但必须确保：采访结束前，Q2/Q3 的信息点被明确问到并得到回应。
"""
...
# 8个“意外感钩子库”（统一放在规则里，供各风格随时调用）
HOOKS_BANK = r"""
【意外感追问钩子库（可在任意轮次调用，但要自然、一次只用一个）】
钩子1：三字存档
- “我想把你的愿望‘存档’一下：如果用三个字概括，会是哪三个字？”
- 追问落地：“这三个字里，哪一个最难？你准备怎么开始（Q3）？”

钩子2：颜色/声音化
- “很怪但很好回答：你的2026愿望如果有颜色/声音，它像什么？”
- 追问落地：“那你希望生活里先出现哪个具体变化来证明它正在发生（Q2）？”

钩子3：第一步小到离谱
- “我们把愿望拆到‘小到离谱’：明天起床后第一件能做的动作是什么（Q3）？”
- 追问落地：“这个动作坚持一周，你觉得生活会先好在哪（Q2）？”

钩子4：新闻标题（适合Q4）
- “如果把你2025那件难忘/自豪的事写成一条新闻标题，你会怎么写？”
- 追问下潜：“标题背后最关键的一幕是什么？你当时站在哪里、谁在场？”

钩子5：一条消息/一个物件（轻松入口）
- “你不用讲大事：2025里一条消息/一个物件，最能代表那段经历的是哪个？”
- 追问下潜：“它让你当时最强烈的感觉是什么？为什么会那样（Q4深挖）？”

钩子6：最想谢谢/最想放过
- “2025结束前，你最想谢谢谁？或者最想放过谁（也可以是自己）？”
- 追问下潜：“这句话你会怎么对TA说？你声音会高一点还是轻一点？”

钩子7：隐藏成就
- “如果2026给你一个隐藏成就，不写在成绩单上，你最希望是什么？”
- 追问落地：“为了拿到它，你生活里最想先改变哪一块（Q2），第一步怎么做（Q3）？”

钩子8：愿望保险箱（自然引到Q5）
- “我想把你的愿望放进‘2026保险箱’，但要一个动作当签名：等会击掌时你想在心里默念哪四个字？”
- 进入互动：“好，那我把手举起来啦——可以和你击个掌吗（Q5）？”
"""
...
PROMPT_POOL = [
    STYLE_0 + Q23_BLEND + HOOKS_BANK + HALF_DUPLEX_RULES + BASE_RULES,
    STYLE_1 + Q23_BLEND + HOOKS_BANK + HALF_DUPLEX_RULES + BASE_RULES,
    STYLE_2 + Q23_BLEND + HOOKS_BANK + HALF_DUPLEX_RULES + BASE_RULES,
    STYLE_3 + Q23_BLEND + HOOKS_BANK + HALF_DUPLEX_RULES + BASE_RULES,
    STYLE_4 + Q23_BLEND + HOOKS_BANK + HALF_DUPLEX_RULES + BASE_RULES,
]
