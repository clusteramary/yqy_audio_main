#!/usr/bin/env bash
# ============================================================
# 启动入口：main.py（本地 PyAudio 扬声器播放）
# 用法：
#   1) 桌面双击 main_本地播放.desktop
#   2) 项目内终端运行: bash run_local_playback.sh
# 日志：logs/run_local_playback_*.log
# ============================================================
set -u
cd "$(dirname "$0")" || exit 1
mkdir -p logs
...
LOG="logs/run_local_playback_$(date +%Y%m%d_%H%M%S).log"
...
if DBG "检查 roscore: $(rostopic list >/dev/null 2>&1 && echo OK || echo FAIL)"; then :; fi
if ! rostopic list >/dev/null 2>&1; then
    echo "[警告] 未检测到 roscore（ROS 主节点），相机话题 /camera/color/image_raw 可能不可用。"
    echo "       若机器人相机未启动，人脸识别将无法工作。"
fi

# ---------- 启动 ----------
DBG "前置检查全部通过，开始启动主程序"
echo "[启动] OUTPUT_AUDIO_MODE=pyaudio  main.py"
echo "[日志] $PWD/$LOG"
echo "[提示] 按 Ctrl+C 停止程序"
OUTPUT_AUDIO_MODE=pyaudio .venv/bin/python -u main.py 2>&1 | tee -a "$LOG"
echo
echo "=== 程序已退出，日志见: $LOG ==="
