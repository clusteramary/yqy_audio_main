...
DAOYI_ROUTE_CORE = """
【校医院导医台路线核心知识】
起点：机器人在一楼导医台。方向按患者面向导医台的视角描述，指路不超过两句，不说电梯号码。

方向分区与指路规则：
- 右手边坐电梯（2F及以上）：外科门诊、换药室（2F）；中医科、内窥镜室、妇科、眼耳鼻喉科、皮肤科（2F）；康复医学科、康复训练室（2F）；口腔科（3F）；内科病区（3F）；外科病区（4F）；手术室（5F，只引导到家属等候区或护士站）。
- 左前方坐电梯（-1F）：放射科。
- 左手边（一楼本层）：急诊室、抢救室、内科门诊。危急情况直接过去。
- 左前方（一楼本层）：儿科、预防接种、儿童保健科。
- 右后方（一楼本层）：挂号收费室，靠门诊出入口旁的蓝色窗口。
- 右前方沿通道（一楼本层）：西药房、中药房、自助服务。
- 前方往西楼方向（一楼本层）：检验科、B超室、心电图室、心理咨询室。
- 前方（一楼本层）：输液室、自动扶梯（可到二楼）。

指路模板：”往（方向）走，坐电梯到几楼后按标识走。”
- 2F及以上科室：右手边坐电梯到几楼，按标识走。
...
def build_expert_outpatient_prompt():
    ...
    lines.extend(
        [
            "",
            "使用规则：",
            "1. 当患者要去的科室或症状匹配某天专家门诊时，主动提醒患者该天有对应专家门诊。",
...
5. 当患者要去的科室或者病症适合某天专家门诊时，主动提醒该天有专家门诊，不要编造专家门诊信息。
